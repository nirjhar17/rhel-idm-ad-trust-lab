---
layout: default
---

# Red Hat IDM + AD Trust Lab

Welcome to the documentation and resources for implementing Red Hat Identity Management with Active Directory Trust.

## Blog Posts

- [How We Solved Linux Access Control for a Bank Using Red Hat IDM and Active Directory Trust](blog-idm-ad-trust-compliance)

## Resources

- [Architecture Overview](ARCHITECTURE)
- [Demo Script](DEMO_SCRIPT)
- [Quick Reference](quick-reference)

## Lab Setup Guides

- [Phase 2: AD Setup](phase2-ad-setup)
- [Phase 3: IDM Primary Setup](phase3-idm-primary)
- [Phase 4: AD Trust Configuration](phase4-ad-trust)
- [Phase 5: IDM Replica Setup](phase5-idm-replica)
- [Phase 6: Client Enrollment](phase6-client-enrollment)
- [Phase 7: RBAC Configuration](phase7-rbac-config)
- [Phase 8: Validation](phase8-validation)

## Presentations

- [Kerberos Trust Flow Slide](slide-kerberos-trust-flow.html) - How IDM and AD communicate
- [Authentication Flow Slide](slide-authentication-flow.html) - How AD users login to Linux
- [Authorization Flow Slide](slide-authorization-flow.html) - What AD users can do on Linux
- [Group Chain Slide](slide-group-chain.html) - How AD groups map to IDM rules
